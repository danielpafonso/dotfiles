...
() {
  ...
  # the code points they use for their special characters.
...
prompt_gcloud_project() {
  if [[ $GCLOUD_PROMPT ]]; then
    ...
    if [[ $GCLOUD_PROJECT  ]]; then
      ...
      project=$(grep "project" ~/.config/gcloud/configurations/config_default| cut -d " " -f 3)
    fi
    if [[ $project ]]; then
      prompt_segment 255 $CURRENT_FG "$project"
    fi
  fi
}
...
prompt_azure_subscription() {
  if [[ $AZURE_PROMPT ]]; then
    ...
    subscription=$(cat ~/.azure/azureProfile.json| jq -c ".subscriptions[]| {name, isDefault} "|grep "isDefault\":true"|jq -r .name)
    if [[ $subscription ]]; then
      prompt_segment 020 white "$subscription"
    fi
  fi
}
